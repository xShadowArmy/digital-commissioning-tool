    ...
    public class MediumWarehouseStrategie : WarehouseStrategieBase
    {
        ...
        public void GenerateWall()
        {
            ...
            for ( int i = 0; i < WallsNorth.Length; i++ )
            {
                ...
                WallsSouth[i] = new ObjectTransformation( new Vector3( xOffset + 40f, yOffset + 0f, zOffset + 30.2f - i ), nsWallRotation, wallScale );
            }

            // EastWest wall
            for ( int i = 0; i < WallsEast.Length; i++ )
            {
                WallsEast[i] = new ObjectTransformation( new Vector3( xOffset + 0.4f + i, yOffset + 0f, zOffset + 30.6f ), ewWallRotation, wallScale );
