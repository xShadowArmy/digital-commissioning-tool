                ...
                for ( int i = 0; i < GameManager.GameContainer.Data.Container.Count; i++ )
                {
                    if ( GameManager.GameContainer.Data.Container[i].ID == GetID( ) )
                    {
                        ...
                        for ( int j = 0; j < Data.Length; j++ )
                        {
                            if ( Data[j] != null )
                            {
                                storage.Items[j] = new ProjectItemData( Data[j].IDRef, Data[j].GetID( ), Data[j].Count, Data[j].Weight, Data[j].Name, Data[j].InQueue, Data[j].QueuePosition, Data[j].ParentItem.GetID( ), Data[j].ParentItem.ParentStorage.GetID( ), new ProjectTransformationData( Data[j].Position, Data[j].Rotation, Data[j].Scale ) );
                            }
                        }
